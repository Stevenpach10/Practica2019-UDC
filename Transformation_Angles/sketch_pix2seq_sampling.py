...
def sampling_conditional_Modified(data_dir, sampling_dir, model_dir):
    ...
    for i in range(total):
        print('Procesando %.2f'%i+' de %.2f' %total)
        image = load_image([directory+images[i]])  # ndarray, [N_points, 3]
        z = encode(image, sess, eval_model)
        strokes_out = decode(sess, sampling_model, eval_model.hps.max_seq_len, z, temperature=0.1)
        cont += strokes_out.shape[0]
        dataset = np.append(dataset, strokes_out.flatten())
    dataset = dataset.reshape((-1,3))
    print(dataset.shape)
    print(dataset)
    np.save('validation',dataset)

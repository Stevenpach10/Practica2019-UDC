...
def load_log():
    ...
    for summary in tf.train.summary_iterator(FLAGS.log_root+"/events.out.tfevents.1575970207.c7260"):
        for v in summary.summary.value:
            ...
            if(v.tag == "Time_Taken_Train"):
                 time_taken_train.append(v.simple_value)
            if(v.tag == "Train_Cost"):
                 train_cost.append(v.simple_value)
            if(v.tag == "Train_Reconstr_Cost"):
                 train_reconstr_cost.append(v.simple_value)
            if(v.tag == "Valid_Cost"):
                 valid_cost.append(v.simple_value)
            if(v.tag == "Valid_Reconstr_Cost"):
                 valid_reconstr_cost.append(v.simple_value)
            if(v.tag == "Eval_Cost"):
                 eval_cost.append(v.simple_value)
            if(v.tag == "Eval_Reconstr_Cost"):
                 eval_reconstr_cost.append(v.simple_value)
        step.append(summary.step)

    learning_rate = np.asarray(learning_rate) 
    time_taken_train = np.asarray(time_taken_train)
    train_cost = np.asarray(train_cost)
    train_reconstr_cost = np.asarray(train_reconstr_cost)
    valid_cost = np.asarray(valid_cost)
    valid_reconstr_cost = np.asarray(valid_reconstr_cost)
    eval_cost = np.asarray(eval_cost)
    eval_reconstr_cost = np.asarray(eval_reconstr_cost)

    print(len(learning_rate))
    print(len(time_taken_train))
    print(len(train_cost))
    print(len(train_reconstr_cost))
    print(len(valid_cost))
    print(len(valid_reconstr_cost))
    print(len(eval_cost))
